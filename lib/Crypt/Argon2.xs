...
SV* argon2d_pass(SV* password, SV* salt, int t_cost, SV* m_factor, int parallelism, size_t output_length, SV* additional = undef, SV* secret = undef)
	ALIAS:
	argon2d_pass = Argon2_d
	argon2i_pass = Argon2_i
	argon2id_pass = Argon2_id
	PREINIT:
	char *password_raw, *salt_raw, *additional_raw, *secret_raw, *output;
	STRLEN password_len, salt_len, additional_len = 0, secret_len = 0;
	...
	secret_raw = SvOK(secret) ? SvPVbyte(secret, secret_len) : NULL;
...
SV* argon2d_raw(SV* password, SV* salt, int t_cost, SV* m_factor, int parallelism, size_t output_length, SV* additional = undef, SV* secret = undef)
	ALIAS:
	argon2d_raw = Argon2_d
	argon2i_raw = Argon2_i
	argon2id_raw = Argon2_id
	PREINIT:
	char *password_raw, *salt_raw, *additional_raw, *secret_raw;
	STRLEN password_len, salt_len, additional_len = 0, secret_len = 0;
	int rc, m_cost;
	CODE:
	m_cost = parse_size(m_factor, ix);
...
SV* argon2d_verify(SV* encoded, SV* password, SV* secret = undef)
	ALIAS:
	argon2d_verify = Argon2_d
	argon2i_verify = Argon2_i
	argon2id_verify = Argon2_id
	PREINIT:
	char *password_raw, *encoded_raw, *secret_raw;
	STRLEN password_len, encoded_len, secret_len = 0;
	int status;
	CODE:
	password_raw = SvPVbyte(password, password_len);
